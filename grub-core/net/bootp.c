...
get_iana_address (const struct grub_dhcpv6_option *popt)
{
...
static const grub_uint8_t*
find_address (const struct grub_net_dhcpv6_packet *packet)
{
				  ...
				  struct grub_dhcpv6_session *session)
{
  ...
  multicast.ipv6[1] = grub_cpu_to_be64 (0x10002ULL);
  ...
  *((grub_uint16_t *) popt->data) = grub_cpu_to_be16 (OPTION_BOOTFILE_URL);
  ...
  popt = (struct grub_dhcpv6_option*) nb->data;
  popt->code = grub_cpu_to_be16 (OPTION_ELAPSED_TIME);
  popt->len = grub_cpu_to_be16 (2);
  ...
  *((grub_uint16_t *) popt->data) = grub_cpu_to_be16 ((grub_uint16_t)elapsed);
  ...
  udph->src = grub_cpu_to_be16 (546);
  udph->dst = grub_cpu_to_be16 (547);
  udph->chksum = 0;
  ...
  for (interval = 200; interval < 10000; interval *= 2)
      ...
      FOR_DHCPV6_SESSIONS (session)
	{
	  ...
	  *((grub_uint16_t *) opt->data) = 0;
	  ...
	  opt = (struct grub_dhcpv6_option *)nb->data;
	  opt->code = grub_cpu_to_be16 (OPTION_CLIENTID); //option_client_id
	  opt->len = grub_cpu_to_be16 (sizeof(*duid));

	  duid = (struct grub_DUID_LL *) opt->data;

	  duid->type = grub_cpu_to_be16 (3) ;
	  duid->hw_type = grub_cpu_to_be16 (1);
	  ...
	  opt = (struct grub_dhcpv6_option *)nb->data;
	  opt->code = grub_cpu_to_be16 (OPTION_IA_NA);
